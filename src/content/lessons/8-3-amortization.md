...
phase: 8
order: 3
title: "Amortization — why your early payments are almost all interest"
...
quiz:
  - q: "What does 'amortization' mean?"
    options:
      - "The interest rate rises over time"
      - "A loan paid off in equal payments, each split between interest and principal — with the split shifting over time"
      - "Skipping payments without penalty"
      - "Paying only the interest forever"
    answer: 1
    explain: "Amortization is the schedule of equal payments that pays a loan to zero. Each payment covers that month's interest first, then the rest goes to principal — and that mix changes as the balance shrinks."
  - q: "On the $20,000 auto loan, how is the very first $396.02 payment split?"
    options:
      - "$396.02 all to principal"
      - "$116.67 interest / $279.35 principal"
      - "$279.35 interest / $116.67 principal"
      - "It's all interest"
    answer: 1
    explain: "Payment one: 7% APR ÷ 12 on the $20,000 balance is $116.67 of interest; the remaining $279.35 chips at the principal. The split-bar shows interest as the larger early slice."
  - q: "Why is so little of an early payment going to principal?"
    options:
      - "The bank keeps the difference as a fee"
      - "Interest is charged on the balance, and early on the balance is at its largest, so the interest slice is biggest"
      - "Early payments are smaller than later ones"
      - "It's random"
    answer: 1
    explain: "Interest is the rate times the current balance. When the balance is highest (the start), the interest portion is biggest, leaving the least for principal. As the balance falls, the interest slice shrinks and principal grows."
  - q: "What does the shifting split tell you about paying extra?"
    options:
      - "Extra payments don't matter until the end"
      - "Extra paid early goes straight to principal, shrinking every future interest charge — so it has outsized impact"
      - "You should wait until the last month to pay extra"
      - "Extra payments increase your interest"
    answer: 1
...
1. **Interest** — that month's cost of borrowing, charged on the *current* balance.
2. **Principal** — whatever's left after the interest, which actually reduces what you owe.
